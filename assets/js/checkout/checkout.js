...
function getAccordianAccounts(data, rebates) {
    ...
    let accordianAccounts = newDataJoined && newDataJoined.map(order => {
        ...
        let everyItemNotContainsFreeGoods = order["product_details"]?.every(item => item.free_goods == "null");
        let everyItemNotContainsPayTerm = order["product_details"]?.every(item => item.payterm == "null");
        let everyItemNotContainsUnits = order["product_details"]?.every(item => item.units == "null");
        let everyItemNotContainsDiscount = order["product_details"]?.every(item => item.discount == "null");
        return `
            <div class="accordion">
                                            ...
                                            <div class="checkout_item_container thead">
                                                ...
                                                <div class="checkout_item_details">
                                                    <div class="item_data_wrapper_row title">
                                                        ${!everyItemNotContainsUnits ? `<div class="item_data">${locale["labels"]["units"]}</div>` : ''}
                                                        ${!everyItemNotContainsFreeGoods ? `<div class="item_data">${locale["labels"]["freeGoods"]}</div>` : ''}
                                                        ${!everyItemNotContainsDiscount ? `<div class="item_data">${rebates ? locale["labels"]["offInvoiceDiscount"] : locale["labels"]["onInvoiceDiscount"]}</div>` : ''}
                                                        ${!everyItemNotContainsPayTerm ? `<div class="item_data">${locale["labels"]["payTerm"]}</div>` : ''}
                                                    </div>
...
function getAccordianAccountsData(data, rebates) {
    ...
    let everyItemNotContainsFreeGoods = data?.every(item => item.free_goods == "null");
    let everyItemNotContainsPayTerm = data?.every(item => item.payterm == "null");
    let everyItemNotContainsUnits = data?.every(item => item.units == "null");
    let everyItemNotContainsDiscount = data?.every(item => item.discount == "null");
                        ...
                        <div class="checkout_item_container">
                            ...
                            <div class="checkout_item_details">
                                <div class="item_data_wrapper_row">
                                    ${!everyItemNotContainsUnits ? `<div class="item_data">${item["units"] !== "null" ? item["units"] : "-"}</div>` : ''}
                                    ${!everyItemNotContainsFreeGoods ? `<div class="item_data">${item["free_goods"] !== "null" ? item["free_goods"] : "-"}</div>` : ''}
                                    ${!everyItemNotContainsDiscount ? `<div class="item_data">${item["discount"] !== "null" ? (item["discount"] + '%') : "-"}</div>` : ''}
                                    ${!everyItemNotContainsPayTerm ? `<div class="item_data">${item["payterm"] !== "null" ? (item["payterm"] + 'D') : "-"}</div>` : ''}
                                </div>
                                <div class="checkout_edit_icon">
                                    <div class="arrow edit quantityEditBackToSelection hint--left hint--rounded hint--bounce" aria-label="Select" brand="${item['brand']}">
                                        ${rebates ? "" : '<img src="/gskPortugalPhase2/assets/images/svg/edit.svg" key=${index} />'}
