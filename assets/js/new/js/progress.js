    ...
    $("#progress_plan_main").prepend(`
        <div class="title">
            ...
            <div class="arrow calendar">
                ...
                <p style="color: #f36633;">${locale["labels"]["endDate"]}:&nbsp;&nbsp;</p> <p class="dateValue">${data["last_date"]}</p>
            </div>
        </div>
...
function getProductsProgress(item, detailed, hideAdd, basicProgress, colorscheme, hideSelectedProgress, initial) {
    ...
    let addBtn = `
        ${basicProgress ?
            ...
            :
            `
                <label class="quantity_value">${parseInt(item["purchased"] || 0) + parseInt(item["selected"]) > parseInt(item["max_limit"]) ? parseInt(item["max_limit"]) : parseInt(item["selected"])}</label>
                <div class="progressbar_wrapper" skudata=${`${item["sku"]}`} style="width: auto;">
    ...
    return `
        <div class="progressbar flex">
            ...
            <div class="wrapper_brand_progress ${detailed ? 'detailed': ''}" style="width: ${detailed ? '75' : basicProgress ? '100' : '85'}%;">
            ...
            <div class="wrapper_brand_progress ${detailed ? 'detailed': ''}" style="width: ${detailed ? '10' : basicProgress ? '0' : '15'}%; padding-left: ${hideAdd ? "0px" : basicProgress ? '0px' : "1%"}; font-size: 12px;">
                ${hideAdd ? `` : addBtn}
            </div>
        </div>
    `;
