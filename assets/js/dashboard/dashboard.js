function loadUserWelcomeUI(data) {
    ...
    let everyItemNotContainsPrice = lastOrder?.["product_details"]?.every(item => item.price == "null");
    let everyItemNotContainsFreeGoods = lastOrder?.["product_details"]?.every(item => item.free_goods == "null");
    let everyItemNotContainsPayTerm = lastOrder?.["product_details"]?.every(item => item.payterm == "null");
    let everyItemNotContainsUnits = lastOrder?.["product_details"]?.every(item => item.units == "null");
    let everyItemNotContainsDiscount = lastOrder?.["product_details"]?.every(item => item.discount == "null");
    ...
    if (prevOrderLength) {
                ...
                <div class="order_cart history hide">
                    ...
                    <div class="detail">
                        <table class="ui very basic table" style="width: 100%;" skudata=${lastOrder["sku"]} date="${lastOrder["ordered_date"]}" orderid=${lastOrder["order_no"]}>
                            <thead>
                                <tr class="info_row">
                                    ${!everyItemNotContainsPrice ? `<td class="info_data" colspan="2">${locale["labels"]["estPrice"]}</td>` : ""}
                                    ${!everyItemNotContainsUnits ? `<td class="info_data" colspan="1">${locale["labels"]["units"]}</td>` : ""}
                                    ${!everyItemNotContainsFreeGoods ? `<td class="info_data" colspan="1">${locale["labels"]["freeGoods"]}</td>` : ""}
                                    ${!everyItemNotContainsDiscount ? `<td class="info_data" colspan="1">${Boolean(lastOrder["on_invoice"]) ? "On Invoice Discount" : "Off Invoice Discount"}</td>` : ""}
                                    ${!everyItemNotContainsPayTerm ? `<td class="info_data" colspan="1">${locale["labels"]["payTerm"]}</td>` : ""}
                                </tr>
                            </thead>
        ...
        data && data["previous_orders"] && data["previous_orders"]["orders"] && data["previous_orders"]["orders"].map((orderData, index) => {
                    ...
                    <div class="order_cart history hide">
                        ...
                        <div class="detail">
                            <table class="ui very basic table" skudata=${orderData["sku"]} orderid=${orderData["order_no"]}>
                                <thead>
                                    <tr class="info_row">
                                        ${!everyItemNotContainsPrice ? `<td class="info_data" colspan="1">${locale["labels"]["estPrice"]}</td>` : ""}
                                        ${!everyItemNotContainsUnits ? `<td class="info_data" colspan="1">${locale["labels"]["units"]}</td>` : ""}
                                        ${!everyItemNotContainsFreeGoods ? `<td class="info_data" colspan="1">${locale["labels"]["freeGoods"]}</td>` : ""}
                                        ${!everyItemNotContainsDiscount ? `<td class="info_data" colspan="2">${Boolean(lastOrder["on_invoice"]) ? "On Invoice Discount" : "Off Invoice Discount"}</td>` : ""}
                                        ${!everyItemNotContainsPayTerm ? `<td class="info_data" colspan="1">${locale["labels"]["payTerm"]}</td>` : ""}
                                    </tr>
                                </thead>
            ...
            $(".card_click").click(function (e) {
                ...
                if (childElement.hasClass("hide")) {
                    ...
                    let everyItemNotContainsPrice = additionalDetails?.every(item => item.price == "null");
                    let everyItemNotContainsFreeGoods = additionalDetails?.every(item => item.free_goods == "null");
                    let everyItemNotContainsPayTerm = additionalDetails?.every(item => item.payterm == "null");
                    let everyItemNotContainsUnits = additionalDetails?.every(item => item.units == "null");
                    let everyItemNotContainsDiscount = additionalDetails?.every(item => item.discount == "null");
                    ...
                    additionalDetails && additionalDetails.map((item, index) => {
                        getTableBodyChildElement.append(`
                            ...
                            <tr class="info_row key${index}">
                                ${!everyItemNotContainsPrice ? `<td class="info_data" colspan="1">€ ${item["price"] !== "null" ? item["price"] : "-"}</td>`: ""}
                                ...
                                ${!everyItemNotContainsFreeGoods ? `<td class="info_data" colspan="1">${item["free_goods"] !== "null" ? item["free_goods"] : "-"}</td>`: ""}
                                ${!everyItemNotContainsDiscount ? `<td class="info_data" colspan="1">${item["discount"] !== "null" ? (item["discount"] + '%') : "-"}</td>`: ""}
                                ${!everyItemNotContainsPayTerm ? `<td class="info_data" colspan="1">${item["payterm"] !== "null" ? (item["payterm"] + 'D') : "-"}</td>`: ""}
                            </tr>
                        `);
                    })
                ...
                if (childElement.hasClass("hide")) {
                    ...
                    additionalDetails && additionalDetails.map((item, index) => {
                        getTableBodyChildElement.append(`
                            ...
                            <tr class="info_row key${index}">
                                ${!everyItemNotContainsPrice ? `<td class="info_data" colspan="1">€ ${item["price"]}</td>` : ""}
                                ${!everyItemNotContainsUnits ? `<td class="info_data editable" colspan="1"><input value=${item["units"]} type="text" size="4" maxlength="4" autocomplete="off"/></td>` : ""}
                                ${!everyItemNotContainsFreeGoods ? `<td class="info_data" colspan="1">+${item["free_goods"]}</td>` : ""}
                                ${!everyItemNotContainsDiscount ? `<td class="info_data" colspan="2">${item["discount"]}%</td>` : ""}
                                ${!everyItemNotContainsPayTerm ? `<td class="info_data" colspan="1">${item["payterm"]} D</td>` : ""}
                            </tr>
                        `);
                    });
        ...
        $("#last_order_history").prepend(`<div class='empty_screen_msg'>${locale["labels"]["emptyOrder"]}`);
        $("#last_order_history").append(`<div class="progress_plan" id="progress_plan_main" style="margin-bottom: 0px"></div>`);
    }
